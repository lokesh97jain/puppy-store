...
This setup would be inexpensive, easy to maintain, and sufficient for small datasets.
...
**puppy_photos table:**
- `id` (primary key)
- `puppyId` (foreign key → puppies.id)
...
Only the **primary photo** would be returned for list views to keep payloads small and fast.
...
**REST API** would be used for simplicity, debuggability, and mobile-friendliness. **GraphQL** would be considered if the data model became more complex with many interrelated entities requiring flexible client-side querying. 
...
- Generate variants on upload: 
  - Thumbnail: 200×200 (for list views)
  - Medium: 800×800 (for detail views)
  - Large: 1600×1600 (for full-screen) 
...
## 9. Authentication Considerations 🔐
...
### Health Checks
- `GET /health` → 200 OK (simple liveness check)
- `GET /health/ready` → Checks DB connection, returns 200 or 503
...
└──────┬──────┘
       ...
       ↓
┌──────────────┐
│  Cloud CDN   │ (Images cached globally)
└──────┬───────┘
       ↓
┌──────────────┐      ┌────────────┐      ┌─────────────┐
│  Cloud Run   │─────→│  Cloud SQL │      │   Cloud     │
│  (REST API)  │      │ (Postgres) │      │  Storage    │
│  Autoscaling │      │  Primary + │      │  (Images)   │
└──────┬───────┘      │  Replicas  │      └─────────────┘
       │              └────────────┘
       ↓
┌──────────────┐
...
1. React app → `GET /puppies`
2. Cloud CDN checks cache → MISS
3. Cloud Run checks Redis → MISS
